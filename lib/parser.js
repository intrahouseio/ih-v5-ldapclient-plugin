...
class Parser {
  constructor(params) {
    ...
    this.userAttributes = {
      ...
      guid: params.attr_user_id || this.defUserAttr('guid'),
      memberOf:'memberOf',
      dn:'dn'
    };
    this.groupAttributes = {
      name: params.attr_group_name || this.defGroupAttr('name'),
      guid: params.attr_group_id || this.defGroupAttr('guid'),
      dn:'dn'
    }
  }
  ...
  getMappedGroups(arr) {
    const f_name = this.groupAttributes.name;
    const f_guid = this.groupAttributes.guid;
    const result = [];
    arr.forEach(item => {
